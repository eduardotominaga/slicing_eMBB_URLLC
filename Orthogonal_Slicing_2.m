...
% case where two active URLLC devices transmit in each minislot.
...
parfor x=1:length(Fu)
    Gu1=Gu1_max(1:Fu(x),:);
    Gu2=Gu2_max(1:Fu(x),:);
    Sigma_u1=Gu1./(1+Gu2);          % SINR - URLLC device 1
    Sigma_u2=Gu2./(1+Gu1);          % SINR - URLLC device 2
    for ru=0:0.01:5
        Du1=zeros(1,N);
        Du2=zeros(1,N);
        for j=1:N
...
%% Computation of rb_sum
rb_sum_2=(F-Fu)*rb_orth;
ru_sum_2=2*ru_max;

%% Plotting the curves
figure(1)
    %plot(rb_sum_2(length(rb_sum_2):-1:1),ru_sum_2(length(ru_sum_2):-1:1),'-o','LineWidth',1.5)
    plot(rb_sum_2,ru_sum_2,'-o','LineWidth',1.5)
    grid on
    %ylim([0 1.2])
...
%% Saving the results
save('Results_1_eMBB_2_URLLC.mat','rb_sum_2','ru_sum_2')