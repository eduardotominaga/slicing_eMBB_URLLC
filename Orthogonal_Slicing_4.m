...
% case where four active URLLC devices transmit in each minislot.
...
Hu4_max=raylrnd(1/sqrt(2),F,N);    % Channel coefficients - URLLC device 4
Gu4_max=Gamma_u*Hu4_max.^2;        % Channel gains - URLLC device 4
...
parfor x=1:length(Fu)
    ...
    for ru=0:0.01:5
        ...
        Sigma_u4=zeros(Fu(x),N);
        for j=1:N
            Sigma_u1(1:Fu(x),j)=Gu1(:,j)./(1+Gu2(:,j)+Gu3(:,j)+Gu4(:,j));          % SINR - URLLC device 1
            Sigma_u2(1:Fu(x),j)=Gu2(:,j)./(1+Gu1(:,j)+Gu3(:,j)+Gu4(:,j));          % SINR - URLLC device 2
            Sigma_u3(1:Fu(x),j)=Gu3(:,j)./(1+Gu1(:,j)+Gu2(:,j)+Gu4(:,j));          % SINR - URLLC device 3
            Sigma_u4(1:Fu(x),j)=Gu4(:,j)./(1+Gu1(:,j)+Gu2(:,j)+Gu3(:,j));          % SINR - URLLC device 4
            ...
            if sum_MutualInfo_u1>sum_MutualInfo_u2 && sum_MutualInfo_u1>sum_MutualInfo_u3 && sum_MutualInfo_u1>sum_MutualInfo_u4
                ...
                if Du1(j)==1
                    Sigma_u2(1:Fu(x),j)=Gu2(:,j)./(1+Gu3(:,j)+Gu4(:,j));          % SINR - URLLC device 1
                    Sigma_u3(1:Fu(x),j)=Gu3(:,j)./(1+Gu2(:,j)+Gu4(:,j));          % SINR - URLLC device 2
                    Sigma_u4(1:Fu(x),j)=Gu4(:,j)./(1+Gu2(:,j)+Gu3(:,j));          % SINR - URLLC device 3
                    sum_MutualInfo_u2=sum(log2(1+Sigma_u2(:,j)));
                    sum_MutualInfo_u3=sum(log2(1+Sigma_u3(:,j)));
                    sum_MutualInfo_u4=sum(log2(1+Sigma_u4(:,j)));
                    if sum_MutualInfo_u2>sum_MutualInfo_u3 && sum_MutualInfo_u2>sum_MutualInfo_u4
                        ...
                        if Du2(j)==1
                            Sigma_u3(1:Fu(x),j)=Gu3(:,j)./(1+Gu4(:,j));               % SINR Update - URLLC device 2
                            Sigma_u4(1:Fu(x),j)=Gu4(:,j)./(1+Gu3(:,j));               % SINR Update - URLLC device 3
                            sum_MutualInfo_u3=sum(log2(1+Sigma_u3(:,j)));
                            sum_MutualInfo_u4=sum(log2(1+Sigma_u4(:,j)));
                            if sum_MutualInfo_u3>sum_MutualInfo_u4
                                Du3(j)=(1/Fu(x))*sum(log2(1+Sigma_u3(:,j)))>=ru;
                                if Du3(j)==1
                                    Du4(j)=(1/Fu(x))*sum(log2(1+Gu4(:,j)))>=ru; 
                                end
                            else
                                Du4(j)=(1/Fu(x))*sum(log2(1+Sigma_u4(:,j)))>=ru;
                                if Du4(j)==1
                    ...
                    elseif sum_MutualInfo_u3>sum_MutualInfo_u2 && sum_MutualInfo_u3>sum_MutualInfo_u4
                        Du3(j)=(1/Fu(x))*sum(log2(1+Sigma_u3(:,j)))>=ru;
                        if Du3(j)==1
                            Sigma_u2(1:Fu(x),j)=Gu2(:,j)./(1+Gu4(:,j));               % SINR Update - URLLC device 2
                            Sigma_u4(1:Fu(x),j)=Gu4(:,j)./(1+Gu2(:,j));               % SINR Update - URLLC device 3
                            sum_MutualInfo_u2=sum(log2(1+Sigma_u2(:,j)));
                            sum_MutualInfo_u4=sum(log2(1+Sigma_u4(:,j)));
            ...
            elseif sum_MutualInfo_u2>sum_MutualInfo_u1 && sum_MutualInfo_u2>sum_MutualInfo_u3 && sum_MutualInfo_u2>sum_MutualInfo_u4
                Du2(j)=(1/Fu(x))*sum(log2(1+Sigma_u2(:,j)))>=ru;
                if Du2(j)==1
                    Sigma_u1(1:Fu(x),j)=Gu1(:,j)./(1+Gu3(:,j)+Gu4(:,j));          % SINR - URLLC device 1
                    Sigma_u3(1:Fu(x),j)=Gu3(:,j)./(1+Gu1(:,j)+Gu4(:,j));          % SINR - URLLC device 2
                    Sigma_u4(1:Fu(x),j)=Gu4(:,j)./(1+Gu1(:,j)+Gu3(:,j));          % SINR - URLLC device 3
                    sum_MutualInfo_u1=sum(log2(1+Sigma_u1(:,j)));
                    sum_MutualInfo_u3=sum(log2(1+Sigma_u3(:,j)));
                    sum_MutualInfo_u4=sum(log2(1+Sigma_u4(:,j)));
                    ...
                    elseif sum_MutualInfo_u3>sum_MutualInfo_u1 && sum_MutualInfo_u3>sum_MutualInfo_u4
                        Du3(j)=(1/Fu(x))*sum(log2(1+Sigma_u3(:,j)))>=ru;
                        if Du3(j)==1
                            Sigma_u1(1:Fu(x),j)=Gu1(:,j)./(1+Gu4(:,j));               % SINR Update - URLLC device 2
                            Sigma_u4(1:Fu(x),j)=Gu4(:,j)./(1+Gu1(:,j));               % SINR Update - URLLC device 3
                            sum_MutualInfo_u1=sum(log2(1+Sigma_u1(:,j)));
                            sum_MutualInfo_u4=sum(log2(1+Sigma_u4(:,j)));
            ...
            elseif sum_MutualInfo_u3>sum_MutualInfo_u1 && sum_MutualInfo_u3>sum_MutualInfo_u2 && sum_MutualInfo_u3>sum_MutualInfo_u4
                Du3(j)=(1/Fu(x))*sum(log2(1+Sigma_u3(:,j)))>=ru;
                if Du3(j)==1
                    Sigma_u1(1:Fu(x),j)=Gu1(:,j)./(1+Gu2(:,j)+Gu4(:,j));          % SINR - URLLC device 1
                    Sigma_u2(1:Fu(x),j)=Gu2(:,j)./(1+Gu1(:,j)+Gu4(:,j));          % SINR - URLLC device 2
                    Sigma_u4(1:Fu(x),j)=Gu4(:,j)./(1+Gu1(:,j)+Gu2(:,j));          % SINR - URLLC device 3
                    sum_MutualInfo_u1=sum(log2(1+Sigma_u1(:,j)));
                    sum_MutualInfo_u2=sum(log2(1+Sigma_u2(:,j)));
                    sum_MutualInfo_u4=sum(log2(1+Sigma_u4(:,j)));
        ...
        Pr_Eu3=1-mean(Du3);
        Pr_Eu4=1-mean(Du4);
        if Pr_Eu1<=Eu && Pr_Eu2<=Eu && Pr_Eu3<=Eu && Pr_Eu4<=Eu
            ru_max(x)=ru;
        else
           break; 
        end
...
rb_sum_4=(F-Fu)*rb_orth;
ru_sum_4=4*ru_max;

%% Plotting the curves
figure(1)
    plot(rb_sum_4(length(rb_sum_4):-1:1),ru_sum_4(length(ru_sum_4):-1:1),'-o','LineWidth',1.5)
    grid on    
...
%% Saving the results
save('Results_1_eMBB_4_URLLC.mat','rb_sum_4','ru_sum_4')
