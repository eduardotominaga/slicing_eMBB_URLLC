...
% the case where two active URLLC devices transmit in each minislot.
...
parfor x=1:length(rb)
    for ru=0:0.01:4
        ...
        for Gb_tar=Gb_min:0.1:Gb_tar_max
            Sigma_u1=Gu1./(1+Gu2+Gb_tar);          % SINR - URLLC device 1
            Sigma_u2=Gu2./(1+Gu1+Gb_tar);          % SINR - URLLC device 2
            Db=zeros(F,N);
            Du1=zeros(1,N);
            Du2=zeros(1,N);
            Pr_Eb=zeros(F,1);
            ...
            if Pr_Eu1<=Eu && Pr_Eu2<=Eu && sum(Pr_Eb(:)<=Eb)==F
                ru_max(x)=ru;
...
rb_sum_2_Non=F*rb;        % There are F eMBB users
ru_sum_2_Non=2*ru_max;    % There are 2 URLLC devices

%% Plotting the curve
figure(1)
    plot(rb_sum_2,ru_sum_2,'LineWidth',1.5)
    grid on    
...
%% Saving the results
save('Results_2_Non.mat','rb_sum_2_Non','ru_sum_2_Non')
